...
//*********START: WEBSERVICE FOR LOGIN***********************
    ...
    function notificationTesting(){
          ...
          //  $result = $this->send_notification_ios($usertoken,$notification_body,$data,$message);     
           $this->fcm->serviceAccountKeyFile = "payrollnew-64d72-firebase-adminsdk-asgls-e9b06064cc.json";
           $this->fcm->projectID = "payrollnew-64d72";
            $response = $this->fcm->sendNotification('ekp2uWT3R6aRxUSQz8hewd:APA91bEpNUdaqojv9vVJntrZ5GxDXKVF_tHUURDlXmZODTsXeaDtvQmoLpSWriu_q8LsMoSe4qI9Zwlj7PVF1wJ638swFvp6J_aeQf7v5D-FjNB9nWGvwzA',"testting 123", "testting 123");
        print_r($response); 
    ...
    public function GenerateOtp(){           
        ...
        if($data)        
        {    
            $mobile_number1 = $data->mobile_number * 1;
             $mobile_number = $this->encryptDecrypt("en",$mobile_number1);
             $mob_code = $data->mob_code;
...
//print_r($sendMail); exit ;
        ...
        list($username, $jsonData) = explode('{', $response1);
        echo '{' . $jsonData;exit;
...
// echo $matches[0];
    ...
    function sendMail($otp="11",$email="navinbcs@gmail.com")
    {
        $data = json_decode(file_get_contents('php://input'));
        $config = Array(
        'protocol' => 'smtp',
    ...
    public function Login()
    {
		...
		if(!empty($result->IdentityNo)){
			$result->IdentityNo = $this->encryptDecrypt("dc",$result->IdentityNo);
		}
                ...
                $subscription = $this->WebserviceModel->checkUserPremiumSubscription($result->PatientId);
                ...
                if(empty($checkDevice->DeviceId) || $notificationDeviceId == $device_id)
                {
                    ...
                    $response['subscription']=isset($subscription)?$subscription:"";
        ...
        foreach($file_uploaded_data as $file ){
            ...
            if(is_array($fileArray)){
                foreach($fileArray as $fileData){
                    $fileArrayAttachemnt[count( $fileArrayAttachemnt)-1]["files"][]="https://internal.ring.healthcare/upload/".$fileData->Filename;
                }
            }
        }
        if($userData)             
    ...
    function android_notification(){
        ...
        if(isset($PatientId)){
            $checkPermaiMRDT = $this->checkPermaiMRDT($PatientId);
        }
        $notification_heading = isset($data->notification_heading)?$data->notification_heading:"Report is ready";        
        ...
        if(isset($checkuserid->Platform) && $checkuserid->Platform == "ios"){
            ...
            if(isset($data->msgCorp)){

            $notification_heading = $data->title;
            $notification_body = $data->msgCorp ;
    ...
    public function send_notification_ios($device_id,$notification_body,$data,$message)
    {
        ...
        // echo "<pre>"; print_r($arrayToSend);exit;
    ...
    CURLOPT_URL => 'https://internalapi.ring.healthcare:8443/api/Register/EncryptDecrypt',
...
//echo $response; exit;
    ...
    function patientSearchByMobileNumber()
    {
        ...
        // {
            if($data){ 
                // $mn = $data->mobile * 1;
                // $name = $this->encryptDecrypt("en",$data->name);
                ...
                foreach($reportList as $file ){
                    ...
                    if(is_array($referralFormArray)){
                        foreach($referralFormArray as $referralData){                       
                            $fileArrayAttachemnt[count( $fileArrayAttachemnt)-1]["EreferralForm"][]="https://internal.ring.healthcare/upload/".$referralData->Filename;
                        }
                    }
                    ...
                    if(is_array($fileAttachmentList)){
                        foreach($fileAttachmentList as $fileData){
                            ...
                            if(isset($jsonFileArr[0]->Filename)){    
                                $fileData->FileAttachments = "https://internal.ring.healthcare/upload/".$jsonFileArr[0]->Filename;
    ...
    public function EreportTransitDataById(){
        ...
        if($data)
        {
            ...
            foreach($reportData as $file ){
                ...
                if(is_array($fileArray)){
                    foreach($fileArray as $fileData){
                        $fileArrayAttachemnt[count( $fileArrayAttachemnt)-1]["FileAttachments"][]="https://internal.ring.healthcare/upload/".$fileData->Filename;
                    }
                }
            }
            if($reportData)             
    ...
    public function generateBase64ToImageLink() 
    {
        ...
        if($data) 
        {
            ...
            if ($success) {
                ...
                $insert = $this->db->insert("SharedFilesDetails",$insertArr);

                $insertArr1 = array("FileName" => $file_name.".".$addExt, "FileLocation" => 'upload/', "FileType" => $addExt, "TimeDuration" => "24", "Link" => $root.$file);
				$this->WebserviceModel->insertTimelineForDeleteFile($insertArr1);
                $response['response_code'] = '1';
                $response['response_message'] = 'Success';
                $response['file_link'] = $root.$file;
                // $response['file_link'] = "https://apimobile.ring.healthcare:5026/Ring_dev/index.php/Webservice/downloadFile?token=".$token;
            } else {
                $response["response_code"] = 2;
                $response["response_message"] = "Failed";
            }
   ...
   public function insertDependentProfile()
	...
	public function deleteDependentProfile(){
        ...
        if($data)
        {
            ...
            // $deleteAlert = $this->WebserviceModel->deleteNotifications($dependentPatientId);
            /************************************************************* */
        ...
        if($data)
        {
            ...
            if(!empty($userLat) && !empty($userLong)){
				...
				foreach($result as $val){
                    if (!property_exists($val, 'distance')) {
			...
			if($result){
                // $sortedData = $this->sortByNearestDistance($result);
				$response['response_code']=1;
				$response['response_message']='Success';
				$response['data']=$result;
    ...
    function sortByNearestDistance(array $data): array {
        usort($data, function($a, $b) {
            // Handle null values (move them last)
            if ($a->totalDistance === null && $b->totalDistance === null) return 0;
            if ($a->totalDistance === null) return 1;
            if ($b->totalDistance === null) return -1;

            // Convert "3599.109 KM" → 3599.109
            $distA = floatval(str_replace(" KM", "", $a->totalDistance));
            $distB = floatval(str_replace(" KM", "", $b->totalDistance));
    ...
    public function convertImageLinkToBase64(){
        ...
        {
            // print_r($data);exit;
            // $imageLink = "https://internal.ring.healthcare/upload/EReportsAttachments/00000/0000001226_aaoddnldododn.png";
            $path = $data->imageLink;
            $link = trim($path);
                    ...
                    foreach($fileAttachmentList as $fileData){
                        ...
                        // echo "<pre>";  print_r($jsonFileArrVal);exit; 
                            if(isset($jsonFileArrVal->Filename)){
                                $fileData->FileAttachments = "https://internal.ring.healthcare/upload/".$jsonFileArrVal->Filename;
                                $fileData->Base64FileAttachments = $this->convertImageLinkToBase64ForFileAttachment($fileData->FileAttachments);
                                $fileArrayAttachemnt[count( $fileArrayAttachemnt)-1]["FileAttachments"][]=$fileData;
                            }
                        }
	...
	public function getDoctorForHospitalsearchPage31Jul25(){
        $data = json_decode(file_get_contents('php://input'));       
        if($data)
        {
            ...
            if($userData){
                foreach($userData as $userDataArr){
                    ...
                    // else if(in_array($tenantId, [1631, 1632, 1633])){
                    //     $userDataArr->Appointment = "enable";
                    //     $userDataArr->Scheduled = 1;
                    // }
                    else{
        ...
        if($data)
        {
            ...
            if($userData){
                foreach($userData as $userDataArr){
                    ...
                    $impData = $this->db->select('*')->from('DoctorImplementation')->where('RingDoctorId',$userDataArr->DoctorId)->where('Scheduled',1)->get()->row();
                    // print_r($impData);
                    if(isset($impData) && !empty($impData)){
                        $userDataArr->Appointment = "enable";
                        $userDataArr->impData = $impData;
                        $userDataArr->Scheduled = $impData->Scheduled;
                    }else{
            ...
            $userData2 = $this->WebserviceModel->getDoctorFromTenantForUniversal($tenantId,$specialityId);
            // print_r($userData2);exit;
            $docDta = array();
            if($userData2){
                foreach($userData2 as $userDataArr2){
                    if($userDataArr2->SecondarySpecialityId){
                       $secondData2 = $this->WebserviceModel->getDoctorFromTenantSec($userDataArr2->DoctorId,$tenantId,$userDataArr2->SecondarySpecialityId); 
						
                       array_push($userData2,$secondData2);
                    }
                    $impData1 = $this->db->select('*')->from('DoctorImplementation')->where('RingDoctorId',$userDataArr2->DoctorId)->get()->row();
                    // print_r($impData1);
                    if(!isset($impData)){
                        $userDataArr2->Appointment = "enable";
                        $userDataArr2->Scheduled = 1;
                        $userDataArr2->DoctorName = trim($userDataArr2->DoctorName);
                        array_push($docDta,$userDataArr2);
                    }else{
                        $userDataArr2->Appointment = "disable";
                        $userDataArr2->Scheduled = 0;
                        $userDataArr2->DoctorName = trim($userDataArr2->DoctorName);
                        array_push($docDta,$userDataArr2);
                    }                   
                }
            ...
            if(isset($userData) && isset($docDta))
            {
               $mergedArray = array_merge($userData, $docDta); 
            }else if(isset($userData) && !isset($docDta)){
               $mergedArray = $userData;
            }else if(!isset($userData) && isset($docDta)){
               $mergedArray = $userData2;
            }
            if($mergedArray)
            {
                ...
                $response['response_code'] = '1';
                $response['response_message'] = 'Success';
                $response['response_data'] = $mergedArray;
                if($tenantDetails){
                    $response['Tenant_data'] = $tenantDetails;
                }
                $response['working_data'] = $workingHTML;
    ...
    public function save_report_data(){
        ...
        if($data)
        {
            $url = 'http://sancyberhad.ddns.net/RING_API_TEST/api/Register/UploadFiles';
            $json = $data;       
            $headers = array('Content-Type: application/json');
            $curl = curl_init();
            curl_setopt_array($curl, array(
    ...
    public function saveReportDataForHISDigitalSenseold1(){
        $data = json_decode(file_get_contents('php://input'));
        if($data)
        {
            $UserId = $data->UserId;
            ...
            $HospitalName = isset($data->HospitalName)?$data->HospitalName:"Columbia Asia Hospital Tebrau"; 
            $Description = $data->Description;
			...
			if($chkPatient){
                ...
                $chkHospital = $this->WebserviceModel->tenantsSearchDataByHospitalName($HospitalName);
    ...
    public function SendBackupToDependentInConvertion($UserId,$SessionId)
    {
        ...
        if($chkSession){
            ...
            /**Create Zip Folder ***************/
                ...
                $link = "https://apimobile.ring.healthcare:5026/Ring_dev/".$new_zip_file;
                $message = $link;
    ...
    public function ringGroupListByPatientIdNew(){
        ...
        if($data)
        {
            ...
            foreach($reportList as $file ){
                ...
                // }
                if($file->IsReferralFormProcessed == 0){
                    if(empty($file->EreferralForm))
                    $referralFormArray = '';
                    else
                    $referralFormArray = json_decode($file->EreferralForm);
                    if(is_array($referralFormArray)){
                        foreach($referralFormArray as $referralData){
                            $fileArrayAttachemnt[count( $fileArrayAttachemnt)-1]["EreferralForm"][]="https://internal.ring.healthcare:8443/upload/".$referralData->Filename;
                        }
                    }
                }
                $fileAttachmentList = $this->WebserviceModel->getFileAttachmentByReportTransitId($file->ReportTransitId,$file->DoctorId);
                if(is_array($fileAttachmentList)){
                    foreach($fileAttachmentList as $fileData){
						$jsonFileArr = json_decode($fileData->FileAttachments);
						if(isset($jsonFileArr[0]->Filename)){
							$fileData->FileAttachments1 = "https://internal.ring.healthcare:8443/upload/".$jsonFileArr[0]->Filename;
                            $fileData->FileAttachments = "https://internal.ring.healthcare/upload/".$jsonFileArr[0]->Filename;
                            $fileData->Base64FileAttachments = $this->convertImageLinkToBase64ForFileAttachment($fileData->FileAttachments1);
							$fileArrayAttachemnt[count( $fileArrayAttachemnt)-1]["FileAttachments"][]=$fileData;
						}
                    }
                }
    ...
    public function convertImageLinkToBase64ForFileAttachment($path){
            ...
            // // Suppress warning and handle failure
            // $data = @file_get_contents($link);

            // if ($data === FALSE) {
            //     // Error logging or fallback
            //     error_log("Could not fetch file from: $link");
            //     return null; // or a default image / error message
            // }
    ...
    {      
        ...
        {    
            ...
            // $sendMail = Utility::callSendMail($email,$message,$subject);
          //  $pat_name = $this->encryptDecrypt("dc",$data->FullName);
            $type = "convertProfileOtp";
            $dataArr = array("name"=>'', "otp"=>$otp);
            $sendMail = Utility::callSendMailWithTemplate($email,$type,$dataArr);
            $response['response_code'] = 1;
            $response['response_message'] = 'Success';
            $response['otp'] = $otp;
    ...
    public function EmailConfirmationRequest()
    {      
        $data = json_decode(file_get_contents('php://input'));
        if($data)    
        {    
            ...
            if($patientdata)             
            {
                $insertArr = array(
                                ...
                                "Email"=>$email, 
                                "NewEmail"=>$new_email);
                $insertId = $this->WebserviceModel->insertChangeEmailRequest($insertArr);    
                $requestId  = $this->encryptDecrypt("en",$insertId);
                $en_requestId = urlencode($requestId);
                $pat_email = $this->encryptDecrypt("dc",$patientdata->Email);
                $url = "url";
                $pat_name = $this->encryptDecrypt("dc",$patientdata->FullName);
                $type = "changeEmailConfirmation";
                $mailDataArr = array("name"=>$pat_name, "patientId"=>$PatientId, "new_email"=>$new_email, "requestId"=>$en_requestId);
                $sendMail = Utility::callSendMailWithTemplate($email,$type,$mailDataArr);

                $response['response_code'] = 1;
                $response['response_message'] = 'Success';
                $response['RequestId'] = $insertId;
            }
            else
            {
                $response['response_code']=2;
    ...
    {   
        ...
        if($data)
        {
            ...
            if($getAdv){
              ...
              // echo "<pre>"; print_r($advVal->StartDate); exit;
                    $insertdate = $advVal->InsertDate;
			
		            $PublishedDate  = strtotime($advVal->PublishedDate); 
			
		            $time = strtotime($insertdate);
                    $today = date('Y-m-d h:i:s');
                     //date_default_timezone_set($timeZone);
                    //$advVal->localInsertDate = date("Y-m-d H:i A", $time);
			        $advVal->localInsertDate = $this->TimeConverion($advVal->InsertDate,$timeZone);
                    //$advVal->PublishedDate1 = date("Y-m-d H:i A", strtotime($advVal->PublishedDate));
			        $advVal->PublishedDate = $this->TimeConverion($advVal->PublishedDate,$timeZone);
                    ...
                    if(isset($advVal->FileAttachments)){
                        $advVal->FileAttachments = json_decode($advVal->FileAttachments,true);
                    }                                
                }
                // exit;
            }          
            ...
            {	
		if(isset($data->companyName) && !empty($data->companyName)){
        		$comFinDataArr = $finalArr;
    		}else{
        		foreach($finalArr as $finVal){
            		if(!in_array($finVal->CustomerCompanyName, array_column($comFinDataArr, 'CustomerCompanyName'))){
                		array_push($comFinDataArr,$finVal); 
            		}
        		}
		}
                $response['response_code'] = '1';
                $response['response_message'] = 'Success';
                $response['response_data'] = $comFinDataArr;
            }
            else
            {
                $response['response_code'] = '2';
                ...
                foreach($getAdv as $advVal){
                    ...
                    if(!empty($advVal->CountryMasterId) && !empty($userData->CountryMasterId) && $advVal->CountryMasterId == $userData->CountryMasterId){
                      ...
                      if(isset($advVal->FileAttachments)){
                        ...
                        $advVal->FileAttachments = json_decode($advVal->FileAttachments,true);
                        foreach($advVal->FileAttachments as &$advFileAttachmentsVal){
                            if(isset($advFileAttachmentsVal['Filename'])){
                                // print_r($advFileAttachmentsVal);exit;
                                $FilenameWithPath = "https://internal.ring.healthcare/upload/".$advFileAttachmentsVal['Filename'];
                                // print_r($FilenameWithPath); 
                                $advFileAttachmentsVal['FilenameWithPath'] = str_replace (' ', '%20', $FilenameWithPath);
                            }
                        }
                    }                                     
                }
	...
	function TimeConverion($UTC_Time,$timezone) {
            ...
            if($getData){
                ...
                foreach($getData as $advVal){
                   ...
                   // print_r($advVal); exit;
                    if(!empty($advVal->CountryMasterId) && !empty($userData->CountryMasterId) && $advVal->CountryMasterId == $userData->CountryMasterId){
                        if(!empty($advVal->EndDate)){
                            if($advVal->StartDate <= $currentDate && $advVal->EndDate >= $currentDate){
				if(!empty($keyword)){
					$advVal->NotificationHeading = str_replace($keyword,"<b style='color:black !important;'>".$keyword."</b>",$advVal->NotificationHeading);
					$advVal->NotificationBody = str_replace($keyword,"<b style='color:black !important;'>".$keyword."</b>",$advVal->NotificationBody);
				}
                                array_push($finalArr,$advVal); 

                            }
                        }else{
                            if($advVal->StartDate <= $currentDate){
				if(!empty($keyword)){
					$advVal->NotificationHeading = str_replace($keyword,"<b style='color:black !important;'>".$keyword."</b>",$advVal->NotificationHeading);
					$advVal->NotificationBody = str_replace($keyword,"<b style='color:black !important;'>".$keyword."</b>",$advVal->NotificationBody);
				}
    ...
    function sendmailwithtemplate(){
        ...
        $otp = rand(100000, 999999);
        $zip = "www.zip_link.com";
        $dataArr = array("name"=>"Ravi", "otp"=>$otp, "dependent"=>"saket@gmail.com", "ZipLink"=>$zip);
        $sendMail = Utility::callSendMailWithTemplate($to,$type,$dataArr);
    }
    ...
    function saveReportDataForHISDigitalSense(){    
	    ...
	    $enData = json_encode($data);
            ...
            if($insert)
            {
		$saveFinReport = $this->saveFinancialReportOfPatient($enData);
    ...
    public function saveFinancialReportOfPatient($data){
        if($data)
        {
            $url = 'https://apimobile.ring.healthcare:5026/Ring_dev/index.php/FinancialService/saveFinancialReportOfPatient';
            $json = $data;   
            $headers = array('Content-Type: application/json');
            $curl = curl_init();
            curl_setopt_array($curl, array(
    ...
    function android_notification_function($PatientId){
        ...
        if($checkuserid->Platform == "ios"){
          ...
          //  $result = $this->send_notification_ios($usertoken,$notification_body,$data,$message);     
            $this->fcm->serviceAccountKeyFile = "ring-4c5e3-firebase-adminsdk-zo3qc-9b789d36b9.json";
            $this->fcm->projectID = "ring-4c5e3";
            $response = $this->fcm->sendNotification($usertoken, 'Report is ready', $notification_body, 'Referal', 'null');
            $insertInTable = $this->InsertInAppNotificationInDB($PatientId, 'Report is ready', $notification_body, 'Referal', 'null', $response, $checkuserid->Platform);
            $notificationArr = array(
                "PatientId" => $PatientId,
                "DeviceID" => $token,
                "notification_body" => $notification_body,
                ...
                if($val->DataJson->FileAttachments){                  
                    foreach($val->DataJson->FileAttachments As $docfile){
                        ...
                        if($docfile->DocumentUrl){
                            $url = 'https://apimobile.ring.healthcare/index.php/Mail/getbase64DataOfFileURL';
                            $json = '{
    ...
    public function confirmMailDependentWithBackup(){
        ...
        if($data)
        {
            ...
            $NewMobNumber = $mn;
            $email = $data->email;
            $isBackup = isset($data->isBackup)?$data->isBackup:'';
            $NewMobCode = $data->newMobileCode; 
    ...
    public function saveDoctorDetailsOnRingServerOLD(){	
		...
		if($data)
		{
			...
			else
			{
				...
				$url = 'https://internalapi.ring.healthcare/api/Register/RegisterDoctor';
				$json = $paramJson;   
				// echo $json;exit;
				$headers = array('Content-Type: application/json');
				$curl = curl_init();
    ...
    public function userConnectionStatusForRingConnect()
    {
        $data = json_decode(file_get_contents('php://input'));
        if($data)
        {         
            ...
            $impId = 1;
            $connection = $this->WebserviceModel->userConnectionStatusForRingConnect($ringId,$impId,$ringGroupId);
            if($connection)
            {          
                $response['response_code'] = '1';
                $response['response_message'] = 'Success';	
    ...
    function updateUserMRNforRingConnect(){
        $data = json_decode(file_get_contents('php://input'));
        if($data){
            $RINGID = $data->patientId;
            $ringGroupId = $data->ringGroupId;
            $impId = 1;
            $check = $this->WebserviceModel->userVerificationForTesting($RINGID,$impId,$ringGroupId);
            if(isset($check) && !empty($check)){
                $updateArr = array("IsConnected"=>1);
                $update = $this->WebserviceModel->UpdateMrnOfUser($updateArr,$check->Id);
                if($update){
    ...
    function getDyanamicCalender()
    {
        ...
        if($data){
            ...
            $month = $data->month;
            $year = $data->year;
            $calanderData = $this->WebserviceModel->getDyanamicCalender($doctorId,$month,$year);
            if($calanderData)
            {
                $response['response_code'] = '1';
                $response['response_message'] = 'Success';
                $response['response_data'] = $calanderData;
            }
            else
            {
                $response['response_code'] = '2';
    ...
    function SendMailToUserAppointmentConfirmation()
    {
        $data = json_decode(file_get_contents('php://input'));
        if($data){
            $userName = $data->userName;
	    ...
	    $appointmentTime = $data->appointmentStartTime;
            $doctorName = $data->doctorName;
            $locationName = $data->locationName;
            $locationAddress = $data->locationAddress;
            $type = "AppointmentConfirmation";
            $dataArr = array("userName"=>$userName, "appointmentDate"=>$appointmentDate, "appointmentTime"=>$appointmentTime, "doctorName"=>$doctorName, "locationName"=>$locationName, "locationAddress"=>$locationAddress);
    ...
    function SendMailToUserAppointmentCancellation()
    {
        $data = json_decode(file_get_contents('php://input'));
        if($data){
            $userName = $data->userName;
            ...
            $dataArr = array("userName"=>$userName, "appointmentDate"=>$appointmentDate, "appointmentTime"=>$appointmentTime, "locationName"=>$locationName);
            $sendMail = Utility::callSendMailWithTemplate($email,$type,$dataArr);
            $response['response_code'] = '1';
            $response['response_message'] = 'Success';
        }
    ...
    function SendMailToUserAppointmentReschedule()
    {
        $data = json_decode(file_get_contents('php://input'));
        if($data){
            $userName = $data->userName;
		    ...
		    $newAppointmentTime = $data->newAppointmentStartTime;
            $doctorName = $data->doctorName;
            $locationName = $data->locationName;
            $locationAddress = $data->locationAddress;
            $type = "AppointmentReschedule";
            $dataArr = array("userName"=>$userName, "appointmentDate"=>$appointmentDate, "appointmentTime"=>$appointmentTime, "newAppointmentDate"=>$newAppointmentDate, "newAppointmentTime"=>$newAppointmentTime, "doctorName"=>$doctorName, "locationName"=>$locationName, "locationAddress"=>$locationAddress);
            $sendMail = Utility::callSendMailWithTemplate($email,$type,$dataArr);
            $response['response_code'] = '1';
            $response['response_message'] = 'Success';
        }
    ...
    function getSubscriptionPlan()
    {
        ...
        "data": [
            ...
            {
            "type": "Yearly – Price per Month (from RWA)",
            "price": "1.90",
            "promotionPrice": "1.90",
            "billingFrequency": "billed annually"
            }
    ...
    function getSubscriptionFAQ()
    {
        ...
        "data": [
            {
            "question": "What is RING Premium?",
            "answer": "RING Premium is a paid subscription plan that provides enhanced features and benefits to our users. It includes auto-backup, unlimited dependents, and more."
            },
            {
            "question": "How much does the premium subscription cost?",
            "answer": "The cost of the premium subscription varies depending on the billing cycle (monthly, bi-annually, annually). You can find detailed pricing information by tapping “Go Premium” and “Change Plan”."
            },
            {
            "question": "How do I subscribe to RING Premium?",
            "answer": "To subscribe to the premium plan, go to the “Upgrade to Premium” section in the app menu. Choose the plan that suits you best (monthly, quarterly, or annually) and follow the prompts to complete the payment."
            }
        ]
        }';
    ...
    function getSubscriptionPremiumBenefits123()
    {
        $data = '{
            "response_code": 1,
            "response_message": "Success",
            "data": [
                {
                "label": "Auto-backup of your medical records"
                },
                {
                "label": "Add unlimited dependents"
                },
                {
                "label": "Enjoy an ad-free experience"
                },
                {
                "label": "Exclusive access to new features"
                },
                {
                "label": "Discounts on Health & Wellness services"
                }
            ]
            }';
    ...
    function getSubscriptionPremiumBenefits()
    {
        $data = json_decode(file_get_contents('php://input'));
        if($data){
            $Id = $data->PremiumServicesMasterId;
            $details = $this->WebserviceModel->getSubscriptionPremiumBenefits($Id);
            if($details)
            {
                $response['response_code'] = '1';
                $response['response_message'] = 'Success';
    ...
    function checkPermaiMRDT($PatientId)
    {
        $patientId = $PatientId;
        $ringGrpId = 109;
        $reportList = $this->WebserviceModel->PermaiMRDTViaPatientId($patientId,$ringGrpId);
        if($reportList){
            $reportList->pMobileNumber = $this->encryptDecrypt("dc",$reportList->pMobileNumber);   
            $reportList->pFname = $this->encryptDecrypt("dc",$reportList->pFname);
            $reportList->pLname = $this->encryptDecrypt("dc",$reportList->pLname); 
            $reportList->pEmail = $this->encryptDecrypt("dc",$reportList->pEmail); 
            //print_r($reportList);exit;
            $insertArray = array( 
                "DoctorName"=>$reportList->doctorName." ".$reportList->LastName,
                "PatientName"=>$reportList->pFname." ".$reportList->pLname,
                "RingId"=>$reportList->PatientId,
                "RecordType"=>"MRDT",
                "ImplementationId"=>4,
                "HospitalName"=>$reportList->TenantName,
                "CMSName"=>"PERMAI",
                "PatientMobileCode"=>$reportList->pMobileCode,
                "PatientMobile"=>$reportList->pMobileNumber,
                "PatientEmail"=>$reportList->pEmail,
                "PatientMRN"=>NULL                           
            );
    ...
    {
        $downTime = $this->WebserviceModel->getAppDownTime();
        if($downTime)
        {
            $sdate_timestamp = strtotime($downTime->StartDate);
            $downTime->StartDate = date("Y-m-d H:i:s", $sdate_timestamp);
            $edate_timestamp = strtotime($downTime->EndDate);
            $downTime->EndDate = date("Y-m-d H:i:s", $edate_timestamp);
            $response['response_code'] = '1';
            $response['response_message'] = 'Success';
            $response['response_data'] = $downTime;
        }
        else
        {
            $response['response_code'] = '2';
    ...
    {
        $data = $this->WebserviceModel->getPremiumServiceMasterData();
        if($data)
        {
            $response['response_code'] = '1';
            $response['response_message'] = 'Success';
    ...
    function getPremiumServiceDetailsById()
    {
        $data = json_decode(file_get_contents('php://input'));
        if($data){
            $Id = $data->PremiumServicesMasterId;
            $details = $this->WebserviceModel->getPremiumServiceDetailsById($Id);
            if($details)
            {
                $response['response_code'] = '1';
                $response['response_message'] = 'Success';
    ...
    function getLastSubscriptionTransactionOfPatient()
    {
        $data = json_decode(file_get_contents('php://input'));
        if($data){
            $PatientId = $data->PatientId;
            $details = $this->WebserviceModel->getLastSubscriptionTransactionOfPatient($PatientId);
            if($details)
            {
                $response['response_code'] = '1';
                $response['response_message'] = 'Success';
    ...
    function unsubscribePatientSubscription()
    {
        $data = json_decode(file_get_contents('php://input'));
        if($data){
            $PatientId = $data->PatientId;
            ...
            $unsubscribe = $this->WebserviceModel->unsubscribePatientSubscription($PatientId,$PackageId,$updateArr);
            if($unsubscribe)
            {
                $response['response_code'] = '1';
                $response['response_message'] = 'Success';
    ...
    function android_notification_ringAdmin(){
        $data = json_decode(file_get_contents('php://input'));
        // print_r($data);exit;
        if($data){
            $blankArr = array();
            ...
            foreach($PatientIds as $patVal){
                if(isset($data->msgCorp)){
                    $notification_heading = $data->title;
                    $notification_body = $data->msgCorp ;
                    $notification_type = "Referal";
                }else{
                    $notification_heading = isset($data->notification_heading)?$data->notification_heading:"Report is ready";        
                    $notification_body = isset($data->notification_body)?$data->notification_body:"Your report has been sent please download it.";
                    $notification_type = isset($data->notification_type)?$data->notification_type:"Referal";
                ...
                $userType = 0; //0 = Patient, 1 = Doctor
                $checkuserid = $this->WebserviceModel->checkuserid($patVal->ring_id,$userType);
                $token = isset($patVal->deviceid)?$patVal->deviceid:0;
                if(isset($checkuserid->Platform) && $checkuserid->Platform == "ios"){
                    $message = array(
                                'title' => $notification_heading, 
                                'body' => $notification_body, 
                    ...
                    $response = $this->fcm->sendNotification($token, $notification_heading, $notification_body, $notification_type, "null");
                    $responseArr = json_decode($response); 
                    $resErr = isset($responseArr->error)?$responseArr->error:"";
                    $resSucc = isset($responseArr->name)?$responseArr->name:"";
                    $returnArr = array("ring_id"=>$patVal->ring_id, "error"=>$resErr, "success"=>$resSucc);
                    array_push($blankArr,$returnArr);
                    $notificationArr = array(
                        "PatientId" => $patVal->ring_id,
                        "DeviceID" => $token,
                        "notification_body" => $notification_body,
                        "NotificationDate" => date("Y-m-d H:i:s"),
                        "CreatedDate" => date("Y-m-d H:i:s")
                    ...
                    $message = array(
                            ...
                            //$this->fcm->serviceAccountKeyFile = "angkasa-f7afd-firebase-adminsdk-3xkel-ea1469ca0d.json";
                            $this->fcm->serviceAccountKeyFile = "angkasa-f7afd-firebase-adminsdk-3xkel-45af39f4bd.json";
                            $this->fcm->projectID = "angkasa-f7afd";
                            $response = $this->fcm->sendNotification($token, $notification_heading, $notification_body, $notification_type, "null");
                            $responseArr = json_decode($response); 
                            $resErr = isset($responseArr->error)?$responseArr->error:"";
                            $resSucc = isset($responseArr->name)?$responseArr->name:"";
    ...
    public function LoginNew()
    {
        ...
        if($data)
        {
            $otp = $data->otp;    
            $device_id = $data->device_id;
            $DeviceUniqeId = $data->DeviceUniqeId;
            $platform = $data->platform;
            $result = $this->WebserviceModel->checkOtp($otp);
            if($result || true)             
            {
                $checkDevice = $this->WebserviceModel->checkLoginDeviceNew($result->PatientId);
                $result->MobileNumber = $this->encryptDecrypt("dc",$result->MobileNumber);
                // $result->FullName = $this->encryptDecrypt("dc",$result->FullName);
                ...
                $UserSettings = $this->WebserviceModel->getUserSettings($result->PatientId,"Patient");
                $subscription = $this->WebserviceModel->checkUserPremiumSubscription($result->PatientId);
                $notificationDeviceId = isset($checkDevice->DeviceUniqeId)?$checkDevice->DeviceUniqeId:"";
                if(empty($checkDevice->DeviceUniqeId) || $notificationDeviceId == $DeviceUniqeId)
                {
                    $kunci = $this->config->item('thekey');
                    $token['id'] = $result->PatientId; 
                    $token['data'] = $result;
                    ...
                    $saveDataArray = array( 
                                "PatientId"=> $result->PatientId,
                                "DeviceId"=>$device_id,
                                "DeviceUniqeId"=>$DeviceUniqeId,
                                "Platform"=>$platform,
                                );
                    $notificationUserType = 0;            
                    $checkuserid = $this->WebserviceModel->checkuserid($result->PatientId,$notificationUserType);
                    if($checkuserid){
                        $updateDeviceToken = $this->WebserviceModel->updateDeviceTokenNew($result->PatientId,$device_id,$platform,$DeviceUniqeId);
                    }else{
                        $insertDeviceToken = $this->WebserviceModel->insertDeviceToken($saveDataArray);
                    }
                    $BackupDataNew = $this->WebserviceModel->checkUserBackup($result->PatientId,'Patient',3);
    ...
    function check_device_token_new(){
        $data = json_decode(file_get_contents('php://input'));
            if($data)
            {
                $device_id = $data->deviceUniqeId;
                $patient_id = $data->patient_id;
                $check = $this->WebserviceModel->check_device_token_new($patient_id,$device_id);
                if($check){
                    $response['response_code']=1;
                    $response['response_message']='Success';
                }else{
    ...
    public function checkDeviceTokenByPatientIdNew(){
        $data = json_decode(file_get_contents('php://input'));
        if($data)
        {
            $device_id = $data->deviceUniqeId;
            $patient_id = $data->patient_id;
            $checkDevice = $this->WebserviceModel->checkDeviceTokenByPatientIdNew($patient_id,$device_id);           
            if($checkDevice)             
            {
                $response['response_code'] = 1;
                $response['response_message'] = 'Success';
    ...
    function update_device_token_new(){
        $data = json_decode(file_get_contents('php://input'));
        if($data)
        {
            $device_id = $data->device_id;
            ...
            $platform = $data->platform;
            $patient_id = $data->patient_id;
            $updateDeviceToken = $this->WebserviceModel->updateDeviceTokenNew($patient_id,$device_id,$platform,$deviceUniqeId);
            if($updateDeviceToken == true){
                $response['response_code']=1;
                $response['response_message']='Success';
            }else{
    ...
    {   
        ...
        if($data)
        {
            ...
            // print_r($userData);exit;
            $currentDate = date('Y-m-d H:i:s');
            $getVoucher = $this->WebserviceModel->geVoucherListing($patientId,$currentDate,$CountryMasterId);
           // echo "<pre>"; print_r($getVoucher); exit;       
            $finalArr['Active'] = array();
            $finalArr['Expire'] = array();
            if($getVoucher){              
                foreach($getVoucher as $advVou){
                    $startDate = $advVou->StartDate;
                    $endDate = $advVou->EndDate;
                    if($startDate <= $currentDate && $endDate >= $currentDate){
                        array_push( $finalArr['Active'],$advVou);
                    }else{
                        array_push( $finalArr['Expire'],$advVou);
                    }
                }
            }                
            if($finalArr)
    ...
    public function checkpremiumUser(){
        ...
        if($data)
        {
            ...
            $checkPre= $this->WebserviceModel->checkpremiumUser($patient_id);           
            if($checkPre)             
            {
                $subscription = $this->WebserviceModel->checkUserPremiumSubscription($patient_id);
                if(isset($subscription) && !empty(isset($subscription))){
                    foreach($subscription as $subVal){
                        $Today = date('Y-m-d');
                        $EndDate = date("Y-m-d H:i:s", strtotime($subVal->EndDate));
                        if(($subVal->IsUnsubscribe == 1) && ($EndDate >= $Today)){
                            $subVal->IsReSubscribe = 1;
                        }else{
                            $subVal->IsReSubscribe = 0;
                        }
                    }
                }
                $response['response_code'] = 1;
                $response['response_message'] = 'Success';
                $response['response_data']= $checkPre;
                $response['subscription']=isset($subscription)?$subscription:"";
            }
            else
            {
    ...
    function voucherDataById()
    {   
        $data = json_decode(file_get_contents('php://input'));
        if($data)
        {
            $Id = $data->Id;           
            $VouData =  $this->WebserviceModel->geVoucherDataById($Id);               
            if($VouData)
            {	
                $response['response_code'] = '1';
                $response['response_message'] = 'Success';
                $response['response_data'] = $VouData;
            }
            else
            {
                $response['response_code'] = '2';
    ...
    function android_notification_mataAdmin(){
        $data = json_decode(file_get_contents('php://input'));
        // print_r($data);exit;
        if($data){
            $blankArr = array();
            ...
            foreach($PatientIds as $patVal){
                ...
                if(isset($checkuserid->Platform) && $checkuserid->Platform == "ios"){
                    ...
                    $message = array(
                            ...
                            $this->fcm->serviceAccountKeyFile = "mata-corporate-firebase-adminsdk-vtaqw-df3df052b7.json";
                            $this->fcm->projectID = "mata-corporate";
                            $response = $this->fcm->sendNotification($token, $notification_heading, $notification_body, $notification_type, "null");
                            $responseArr = json_decode($response); 
                            $resErr = isset($responseArr->error)?$responseArr->error:"";
                            $resSucc = isset($responseArr->name)?$responseArr->name:"";
    ...
    function android_notification_polityAdmin(){
        $data = json_decode(file_get_contents('php://input'));
        // print_r($data);exit;
        if($data){
            $blankArr = array();
            ...
            foreach($PatientIds as $patVal){
                ...
                if(isset($checkuserid->Platform) && $checkuserid->Platform == "ios"){
                    ...
                    $message = array(
                            ...
                            $this->fcm->serviceAccountKeyFile = "poltical-corporate-firebase-adminsdk-feect-de0e3c73ac.json";
                            $this->fcm->projectID = "poltical-corporate";
                            $response = $this->fcm->sendNotification($token, $notification_heading, $notification_body, $notification_type, "null");
                            $responseArr = json_decode($response); 
                            $resErr = isset($responseArr->error)?$responseArr->error:"";
                            $resSucc = isset($responseArr->name)?$responseArr->name:"";
    ...
    function getChangeEmailRequestStatus()
    { 
        if ($this->input->server('REQUEST_METHOD') == 'OPTIONS') {
            $data["status"] = "ok";
            echo json_encode($data);
        ...
        {
            $Id = $data->requestId;           
            $ResData =  $this->WebserviceModel->getChangeEmailRequestStatus($Id);
            // print_r($ResData);exit;  
            if($ResData){
                if($ResData->IsAccepted == 1){
                    $ResData->Status = "Accepted" ;
                }else if($ResData->IsCancelled == 1){
                    $ResData->Status = "Cancelled" ;
                }

                $startDate = new DateTime($ResData->InsertDate);
                $endDate = new DateTime($ResData->ExpiryDate);
                $now = new DateTime();
                ...
                if ($now >= $startDate && $now <= $endDate) {
                    $ResData->Countdown = $remaining->format('%h : %i : %s');
                }else {
                    $ResData->Countdown =  "00 : 00 : 00";
            ...
            {	
                $response['response_code'] = '1';
                $response['response_message'] = 'Success';
                $response['response_data'] = $ResData;
            }
            else
            {
                $response['response_code'] = '2';
    ...
    function EmailConfirmationAccepted()
    {   
        if($_GET)
        {
            $Id = $this->encryptDecrypt("dc", $_GET['requestId']);          
            $ResData =  $this->WebserviceModel->getChangeEmailRequestStatus($Id);   
                       
            if($ResData)
            { 
                // print_r($ResData);exit;
                if(($ResData->IsAccepted == 0) && ($ResData->IsCancelled == 0)){
                    // echo 1;exit;
                    $email_en = $this->encryptDecrypt("en",$ResData->NewEmail);
                    $updateDataArray = array(                                
                        "Email"=>$email_en,
                        "UpdateDate"=>date("Y-m-d H:i:s")                               
                    );
                    ...
                    $result = $this->WebserviceModel->updateUserData($updateDataArray,$ResData->PatientId); 
                    if($result)             
                    {    
                        $this->WebserviceModel->updateEmailChangeStatus($updateArr,$Id);                
                        $patientdata = $this->WebserviceModel->getUserDataById($ResData->PatientId);
                        $pat_email = $this->encryptDecrypt("dc",$patientdata->Email);
                        $pat_name = $this->encryptDecrypt("dc",$patientdata->FullName);
                        $type = "changeEmailFinal";
                        $dataArr = array("name"=>$pat_name, "new_email"=>$pat_email);
                        $sendMail = Utility::callSendMailWithTemplate($pat_email,$type,$dataArr);
                        redirect('https://apimobile.ring.healthcare/Ring_dev/index.php/Webservice/CallEmailChangeSuccessPage');exit;
                    }
                }
    ...
    public function CallEmailChangeSuccessPage(){
       $this->load->view('email_change_success');
    }

    function EmailConfirmationCancelled()
    {   
        if($_GET)
        {
            $Id = $this->encryptDecrypt("dc", $_GET['requestId']);          
            ...
            { 
                ...
                if(($ResData->IsAccepted == 0) && ($ResData->IsCancelled == 0)){                    
                    ...
                    $this->WebserviceModel->updateEmailChangeStatus($updateArr,$Id);                
                    redirect('https://apimobile.ring.healthcare/Ring_dev/index.php/Webservice/CallEmailChangeCancelPage');exit;
                }
            }
            else
            {
    ...
    public function CallEmailChangeCancelPage(){
       $this->load->view('email_change_cancel');
    }

    function EmailChangerRequestCancelled()
    {   
        $data = json_decode(file_get_contents('php://input'));
        if($data)
        {
            ...
            { 
                ...
                if(($ResData->IsAccepted == 0) && ($ResData->IsCancelled == 0)){                    
                    ...
                    $response['response_message'] = 'Success';
                    $response['response_data'] = $ResData;
                }else if(($ResData->IsAccepted == 1) && ($ResData->IsCancelled == 0)){
                    $response['response_code'] = '4';
                    $response['response_message'] = 'Request already accepted';
                    $response['response_data'] = $ResData;
                }else if(($ResData->IsAccepted == 0) && ($ResData->IsCancelled == 1)){
                    $response['response_code'] = '5';
                    $response['response_message'] = 'Request already cancelled';
                    $response['response_data'] = $ResData;
                }
            }
            else
    ...
    public function EreportTransitListByReportTransitId(){
        $data = json_decode(file_get_contents('php://input'));
        if($data)
        {
            $ReportTransitId = $data->ReportTransitId;
            $DoctorId = $data->DoctorId;
            $reportList = $this->WebserviceModel->EreportTransitListByReportTransitId($ReportTransitId,$DoctorId);
            $fileArrayAttachemnt = array();
            foreach($reportList as $file ){
                // print_r($file);exit;
                $InsertDate = $file->InsertDate;
                ...
                //     }
                // }
                if($file->IsReferralFormProcessed == 1){
                    if(empty($file->EreferralForm))
                    $referralFormArray = '';
                    else
                    $referralFormArray = json_decode($file->EreferralForm);
                ...
                $fileAttachmentList = $this->WebserviceModel->getFileAttachmentByReportTransitIdNew($file->ReportTransitId,$file->DoctorId);
                if(is_array($fileAttachmentList)){
                    foreach($fileAttachmentList as $fileData){
						$jsonFileArr = json_decode($fileData->FileAttachments);
						if(isset($jsonFileArr[0]->Filename)){
    ...
    public function InAppNotificationListOfPatient(){
        $data = json_decode(file_get_contents('php://input'));
        if($data)
        {
            $PatientId = $data->PatientId;
            $listData = $this->WebserviceModel->InAppNotificationListOfPatient($PatientId);
            if($listData)             
            {
                $response['response_code'] = 1;
                $response['response_message'] = 'Success';
                $response['response_data']= $listData;
            }
            else
            {
                $response['response_code'] = 2;
    ...
    function updateInAppNotificationReadStatus(){
        $data = json_decode(file_get_contents('php://input'));
        if($data)
        {
            $Id = $data->Id;
            $updateArr = array("ReadStatus"=> 1);
            $update = $this->WebserviceModel->updateInAppNotificationReadStatus($Id,$updateArr);
            if($update == true){
                $response['response_code'] = 1;
                $response['response_message'] ='Success';
            }else{
    ...
    function countOfInAPPNotification(){
        $data = json_decode(file_get_contents('php://input'));
        if($data){
            $PatientId = $data->PatientId;
            $count = $this->WebserviceModel->countOfInAPPNotification($PatientId);
            if($count)
            {
                $response['response_code'] = '1';
                $response['response_message'] = 'Success';
    ...
    function deleteInAppNotification(){
        $data = json_decode(file_get_contents('php://input'));
        if($data){
            $Id = $data->Id;
            $delete = $this->WebserviceModel->deleteInAppNotification($Id);
            if($delete)
            {
                $response['response_code'] = '1';
                $response['response_message'] = 'Success';
    ...
    public function get_doctor_availability11() {
        $data = json_decode(file_get_contents('php://input'));
        $userId = $data->userId; 
        $date   = $data->date;
        $interval = isset($data->interval)?($data->interval): 15; 

        if (!$userId || !$date) {
            echo json_encode(['status' => false, 'message' => 'User ID and Date are required']);
            return;
        }

        $scheduleData = $this->WebserviceModel->get_doctor_schedule_details($userId, $date);
        $doctorName = isset($scheduleData['doctor_info']['DisplayName']) 
                      ? $scheduleData['doctor_info']['DisplayName'] 
                      : 'Unknown Doctor';
        if (empty($scheduleData['working_hours'])) {
            echo json_encode([
                'status' => true,
                'doctor_name' => $doctorName,
                'data' => [
                    'message' => 'Doctor is off duty',
                    'available_slots' => [],
                    'booked_slots' => []
                ]
            ]);
       ...
       $shift = $scheduleData['working_hours'][0];

        $startTime = $this->_format_time($shift['FromTime']);
        $endTime   = $this->_format_time($shift['ToTime']);
        $allSlots = $this->_generate_time_slots($date, $startTime, $endTime, $interval);
        ...
        foreach ($allSlots as $slot) {
            ...
            foreach ($scheduleData['appointments'] as $appt) {
                $apptStart = strtotime($date . ' ' . $this->_format_time($appt['FromTime']));
                $apptEnd   = strtotime($date . ' ' . $this->_format_time($appt['ToTime']));

                if (($slotStart < $apptEnd) && ($slotEnd > $apptStart)) {
                    $isBooked = true;
                    if(isset($FirstName) && isset($LastName)){
                        $FirstName = $this->encryptDecrypt("dc",$appt['FirstName']);
                        $LastName = $this->encryptDecrypt("dc",$appt['LastName']);
            ...
            if (!$isBooked) {
                foreach ($scheduleData['leaves'] as $leave) {
                    $leaveStart = strtotime($date . ' ' . $this->_format_time($leave['StartTime']));
                    $leaveEnd   = strtotime($date . ' ' . $this->_format_time($leave['EndTime']));
    ...
    function _generate_time_slots11($date, $start_time, $end_time, $interval) {
        $slots = [];
        ...
        while ($current < $end) {
            ...
            $nextSlot  = strtotime("+$interval minutes", $current);
            
            if ($nextSlot > $end) break; 

            $slotEnd = date('H:i', $nextSlot);
    ...
    function _format_time11($timeStr) {
    ...
    private function _format_time($timeStr) {
        if (!$timeStr) return "00:00";
    ...
    private function _generate_time_slots($date, $start_time, $end_time, $interval) {
        $slots = [];
        $current = strtotime($date . ' ' . $start_time);
        $end     = strtotime($date . ' ' . $end_time);
    ...
    public function get_doctor_availability() {
        ...
        $userId = $data->userId; 
        $rawDate = $data->date; 
        $interval = isset($data->interval) ? (int)$data->interval : 15; 
        ...
        foreach ($scheduleData['working_hours'] as $shift) {
            $startTime = $this->_format_time($shift['FromTime']);
            $endTime   = $this->_format_time($shift['ToTime']);
            
            $shiftSlots = $this->_generate_time_slots($date, $startTime, $endTime, $interval);
            $allGeneratedSlots = array_merge($allGeneratedSlots, $shiftSlots);
        }
        ...
        foreach ($allGeneratedSlots as $slot) {
            ...
            // --- 3.1 Check Appointments ---
            if (!empty($scheduleData['appointments'])) {
                foreach ($scheduleData['appointments'] as $appt) {
                    
                    $apptTimeStrFrom = $this->_format_time($appt['FromTime']);
                    $apptTimeStrTo   = $this->_format_time($appt['ToTime']);

                    $apptStart = strtotime($date . ' ' . $apptTimeStrFrom);
                    $apptEnd   = strtotime($date . ' ' . $apptTimeStrTo);

                    if (($slotStart < $apptEnd) && ($slotEnd > $apptStart)) {
                        $isBooked = true;
                        
                        $patientName = "";
                        if(isset($appt['FullName']) && isset($appt['LastName'])){
                             try {
                                $FirstName = $this->encryptDecrypt("dc", $appt['FullName']); 
                                $LastName  = $this->encryptDecrypt("dc", $appt['LastName']);
                                $patientName = trim($FirstName . ' ' . $LastName);
                             } catch (Exception $e) {
                        ...
                        $bookedSlots[] = [
                            'time' => $slot['start'],
                            'reason' => 'Appointment',
                            'ref_no' => isset($appt['AppointmentNo']) ? $appt['AppointmentNo'] : '',
                            'patient_name' => $patientName ? $patientName : 'N/A',
                            'enrollmentID' => isset($appt['enrollmentID']) ? $appt['enrollmentID'] : '',
                        ];
            ...
            if (!$isBooked && !empty($scheduleData['leaves'])) {
                foreach ($scheduleData['leaves'] as $leave) {
                    $leaveTimeStrFrom = $this->_format_time($leave['StartTime']);
                    $leaveTimeStrTo   = $this->_format_time($leave['EndTime']);

                    $leaveStart = strtotime($date . ' ' . $leaveTimeStrFrom);
                    $leaveEnd   = strtotime($date . ' ' . $leaveTimeStrTo);
        ...
        $finalBookedSlots = array_values(array_unique($bookedSlots, SORT_REGULAR));
        $finalAvailableSlots = array_values(array_unique($availableSlots));
    ...
    public function docAvailTenant() {
        $data = json_decode(file_get_contents('php://input'));
        $headers = $_SERVER["HTTP_AUTHORIZATION"];
        $token = str_replace("Bearer ", "", $headers);
        $kunci = $this->config->item('thekey');
        ...
        $scheduleData = $this->WebserviceModel->get_doctor_schedule_details_copy($userId, $TenantId, $date);
