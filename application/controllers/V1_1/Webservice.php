...
//*********START: WEBSERVICE FOR LOGIN***********************
    ...
    {        
        ...
        if($data)
        {
            ...
            $country = isset($data->country_id)?$data->country_id:0;
            $state = isset($data->state_id)?$data->state_id:'';
            $city = isset($data->city_id)?$data->city_id:'';
    ...
    public function getDependentProfile(){
        ...
        if($data)
        {
			...
			$DependentProfile->IdentityNo = $this->encryptDecrypt("dc",$DependentProfile->IdentityNo);   
    ...
    public function getUserDataById(){   
        ...
        {           
	        ...
	        $userData->IdentityNo = $this->encryptDecrypt("dc",$userData->IdentityNo);   
            ...
            if($userData )             
            {
                $subscription = $this->WebserviceModel->checkUserPremiumSubscription($userData->PatientId);
    ...
    public function updateUserIdentityData(){
        if($this->input->server('REQUEST_METHOD') == 'OPTIONS') {
            $data["status"] = "ok";
            echo json_encode($data);
            exit;
        ...
        $IdentificationType = isset($data->IdentificationTypeId)?$data->IdentificationTypeId:NULL;
        $IdentificationNumber = isset($data->IdentityNo)?$data->IdentityNo:NULL;
        if(!empty($IdentificationType) && !empty($IdentificationNumber)){
            $IdentificationNumber = $this->encryptDecrypt("en",$data->IdentityNo);
        }
        $saveDataArray = array(         
                            "IdentificationTypeId"=>$IdentificationType,          
                            "IdentityNo"=>$IdentificationNumber,
    ...
    {        
        ...
        if($data)
        {
            ...
            $insert = $this->WebserviceModel->insertUserToken($saveDataArr);
            if($insert)
            {
                $response['response_code'] = '1';
                $response['response_message'] = 'Success';
    ...
    public function fetchUserDetailsByToken()
    {        
        $data = json_decode(file_get_contents('php://input'));        
        if($data)
        {
            $token = $data->token;
            $details = $this->WebserviceModel->fetchUserDetailsByToken($token);
            if($details)
            {
                $response['response_code'] = '1';
                $response['response_message'] = 'Success';
    ...
    function UserSignUpWithLogin(){        
        ...
        if($data)
        {
            ...
            $DeviceUniqeId = isset($data->DeviceUniqeId)?$data->DeviceUniqeId:"NULL";
            $saveDataArray = array( 
                                    "MobileNumber"=>$mobile_number,
                                    "FullName"=>$full_name,
                                    "LastName"=>$lastname,
            ...
            {
                $getData = $this->WebserviceModel->signUp($saveDataArray);
                // $login = $this->signupWithLogin($getData, $device_id, $platform, $DeviceUniqeId);
                if($getData)
                {
                    ...
                    // $response['Data'] = $getData;
                    $patientId = $getData;
                    $result = $this->WebserviceModel->checkPatientById($patientId);
                    if($result || true)             
                    {
                        $checkDevice = $this->WebserviceModel->checkLoginDeviceNew($patientId);
    ...
    public function signupWithLogin($patientId, $device_id, $platform, $DeviceUniqeId){
        $result = $this->WebserviceModel->checkPatientById($patientId);
        if($result || true)             
        {
            $checkDevice = $this->WebserviceModel->checkLoginDeviceNew($patientId);
