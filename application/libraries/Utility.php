...
class Utility {
	...
	static function sendMailFromUtility($to,$massage,$subject){ 
	   // echo 'sendMailFromUtility'; exit;
		...
		$config = Array(
    		...
    		'smtp_port' => 587,
            'smtp_user' => 'sendermail2024@gmail.com',
            'smtp_pass' => 'cjunnlivouupmngp',
    		'mailtype'  => 'html',
    		'charset'   => 'iso-8859-1',
    		'smtp_crypto' =>  'tls',
    		'smtp_timeout' => 30
	...
	static function callSendMailNew($to,$emailMessage,$subject){
        ...
        $CI = & get_instance();
        $sendinblueKey = $CI->config->item('sendinblue_api_key');

    	$headers = array('api-key: '.$sendinblueKey,
	...
	static function callSendMailWithTemplate222222($to,$type,$dataArray){
        // $ci = & get_instance();
        // $ch = curl_init();
        // $url = "http://k2key.in/ring/index.php/Mail/sendMailForTesting";
		// $bccArray = "";
		...
		$ch = curl_init();
			curl_setopt_array($ch, array(
				CURLOPT_URL => 'https://apimobile.ring.healthcare/index.php/Mail/sendMailForTesting?' . http_build_query(array(
					'to' => $to,
					'type' => $type,
					'dataArray' => $dataArray
				)),
...
echo $result;exit;
	...
	static function callSendMailWithTemplate11111111($to,$type,$dataArray){
        // $ci = & get_instance();
        // $ch = curl_init();
        // $url="https://apimobile.ring.healthcare:5026/index.php/Mail/sendMailForTesting";
		$headers = array('Content-Type: application/json');
		$bccArray = "";
        $fields = array(
            'to' => $to,
		...
		curl_setopt_array($curl, array(
		CURLOPT_URL => 'https://apimobile.ring.healthcare:5026/index.php/Mail/sendMailForTesting',
		CURLOPT_RETURNTRANSFER => true,
		CURLOPT_ENCODING => '',
		CURLOPT_MAXREDIRS => 10,
		CURLOPT_TIMEOUT => 0,
	...
	static function callSendMailWithTemplate($to,$type,$dataArray){
		...
		// $config['smtp_user'] = 'hello.ringhealthcare@gmail.com';
		// $config['smtp_pass'] = 'qydy eddu izlk pbqi';
        $config['smtp_host'] = 'mail.ring.solutions';
		$config['smtp_user'] = 'hello@ring.solutions';
		$config['smtp_pass'] = 'riso@hello2025';
        ...
        if($emailTemplate){
            if($type == "signUpOtp"){
                ...
                //$emailTemplate->subject = isset($emailTemplate->subject)?$emailTemplate->subject:'Login OTP';
                $emailTemplate->Subject = $emailTemplate->Subject;
                ...
                $emailMsg = str_replace(
                    array('{name}', '{mobile}', '{ZipLink}'),
                    array($dataArray['name'], $dataArray['mobile'], $dataArray['ZipLink']),
                    $emailTemplate->Description
                );
                $emailTemplate->Subject = $emailTemplate->Subject;
                ...
                $emailTemplate->Subject = $emailTemplate->Subject;
            }else if($type == "convertProfileFinalNew"){
                $emailMsg = str_replace(
                    array('{name}', '{mobile}', '{ZipLink}'),
                    array($dataArray['name'], $dataArray['mobile'], $dataArray['ZipLink']),
                    $emailTemplate->Description
                ...
                $emailTemplate->Subject = $emailTemplate->Subject;
            }else if($type == "AppointmentNotification"){
                $emailMsg = str_replace(
                    array('{name}', '{mobile}', '{ZipLink}'),
                    array($dataArray['name'], $dataArray['mobile'], $dataArray['ZipLink']),
                    $emailTemplate->Description
                ...
                $emailTemplate->Subject = $emailTemplate->Subject;
            }else if($type == "AppointmentCancelError"){
                 $emailMsg = str_replace(
                    array('{CMSName}', '{error}', '{date}', '{time}'),
                    array($dataArray['CMSName'], $dataArray['error'], $dataArray['date'], $dataArray['time']),
                    $emailTemplate->Description
                );
                $emailTemplate->Subject = $dataArray['CMSName']." Issue";

                // $emailTemplate->subject = str_replace(
                //     array('{CMSName}'),
                //     array($dataArray['CMSName']),
                //     $emailTemplate->subject
                // );
            }else if($type == "AppointmentRescheduleError"){
                $emailMsg = str_replace(
                    array('{CMSName}', '{error}', '{date}', '{time}'),
                    array($dataArray['CMSName'], $dataArray['error'], $dataArray['date'], $dataArray['time']),
                    $emailTemplate->Description
                ...
                $emailMsg = str_replace(
                    array('{name}', '{AppointmentDate}', '{AppointmentTime}', '{doctorName}', '{locationName}', '{locationAddress}'),
                    array($dataArray['userName'], $dataArray['appointmentDate'], $dataArray['appointmentTime'], $dataArray['doctorName'], $dataArray['locationName'], $dataArray['locationAddress']),
                    $emailTemplate->Description
                );
                $emailTemplate->Subject = $dataArray['locationName']." Appointment Confirmation";

            }else if($type == "AppointmentCancel"){
                $emailMsg = str_replace(
                    array('{name}', '{AppointmentDate}', '{AppointmentTime}', '{locationName}'),
                    array($dataArray['userName'], $dataArray['appointmentDate'], $dataArray['appointmentTime'], $dataArray['locationName']),
                    $emailTemplate->Description
                );
                $emailTemplate->Subject = $dataArray['locationName']." Appointment Cancellation";

            }else if($type == "AppointmentReschedule"){
                $emailMsg = str_replace(
                    array('{name}', '{AppointmentDate}', '{AppointmentTime}', '{doctorName}', '{locationName}', '{locationAddress}', '{newAppointmentDate}', '{newAppointmentTime}'),
                    array($dataArray['userName'], $dataArray['appointmentDate'], $dataArray['appointmentTime'], $dataArray['doctorName'], $dataArray['locationName'], $dataArray['locationAddress'], $dataArray['newAppointmentDate'], $dataArray['newAppointmentTime']),
                    $emailTemplate->Description
                );
                $emailTemplate->Subject = $dataArray['locationName']." Appointment Rescheduled";

            }else if($type == "AppointmentCancelApp"){
                $emailMsg = str_replace(
                    array('{name}', '{AppointmentDate}', '{AppointmentTime}', '{locationName}'),
                    array($dataArray['userName'], $dataArray['appointmentDate'], $dataArray['appointmentTime'], $dataArray['locationName']),
                    $emailTemplate->Description
            ...
            }else if($type == "AppointmentRescheduleApp"){
                $emailMsg = str_replace(
                    array('{name}', '{AppointmentDate}', '{AppointmentTime}', '{doctorName}', '{locationName}', '{locationAddress}', '{newAppointmentDate}', '{newAppointmentTime}'),
                    array($dataArray['userName'], $dataArray['appointmentDate'], $dataArray['appointmentTime'], $dataArray['doctorName'], $dataArray['locationName'], $dataArray['locationAddress'], $dataArray['newAppointmentDate'], $dataArray['newAppointmentTime']),
                    $emailTemplate->Description
            ...
            }else if($type == "RingPremiumSuccess"){
                $emailMsg = str_replace(
                    array('{name}', '{renewal_date}'),
                    array($dataArray['name'], $dataArray['renewal_date']),
                    $emailTemplate->Description
                );
                $emailTemplate->Subject = $emailTemplate->Subject;
            }else if($type == "changeEmailConfirmation"){
                $emailMsg = str_replace(
                    array('{name}', '{new_email}', '{patientId}', '{requestId}'),
                    array($dataArray['name'], $dataArray['new_email'], $dataArray['patientId'], $dataArray['requestId']),
                    $emailTemplate->Description
                );
                $emailTemplate->Subject = $emailTemplate->Subject;
            }else if($type == "AppointmentBookWithPayment"){
                $emailMsg = str_replace(
                    array('{name}', '{appointmentDate}', '{appointmentTime}', '{doctorName}', '{tenantName}'),
                    array($dataArray['userName'], $dataArray['appointmentDate'], $dataArray['appointmentTime'], $dataArray['doctorName'], $dataArray['locationName']),
                    $emailTemplate->Description
                );
                $emailTemplate->Subject = $emailTemplate->Subject;
            }else if($type == "PaymentUnsuccessful"){
                $emailMsg = str_replace(
                    array('{name}', '{appointmentDate}', '{appointmentTime}', '{doctorName}', '{tenantName}'),
                    array($dataArray['userName'], $dataArray['appointmentDate'], $dataArray['appointmentTime'], $dataArray['doctorName'], $dataArray['locationName']),
                    $emailTemplate->Description
                ...
                $emailTemplate->Subject = $emailTemplate->Subject;
            }else if($type == "signUpCareConnect"){
                $emailMsg = str_replace(
                    array('{name}'),
            ...
            if(true){
                ...
                $ci->email->from('hello@ring.solutions', 'RING');
				// $ci->email->to('mishraravi520@gmail.com');
				$ci->email->to($to);
                $ci->email->reply_to('hello@ring.solutions', 'RING');
