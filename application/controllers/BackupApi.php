...
/*********START: WEBSERVICE FOR BACKUP DIRECT FROM DEVICE*************/
    ...
    public function saveBackupReportInTable(){
        ...
        if($data)
        {
            ...
            if(isset($data->EreferralFormBase64) && !empty($data->EreferralFormBase64)){
                /******create file text*********/                     
            ...
            $BackupType = isset($data->BackupType)?$data->BackupType:1;
			 if($SessionId){
        ...
        if($data)
        {
            ...
            if($chkSession){
                ...
                /**Create Zip Folder ***************/
                    ...
                    $link = "https://apimobile.ring.healthcare/Ring_dev/".$new_zip_file;
					$message = $link;
    ...
    public function unzipJsonFile(){
        ...
        if($file_dir)
        {
			...
			$tempArray2['AppointmentHistory'] = json_decode($tempArray2['AppointmentHistory'],true);
        ...
        if($data)
        {
            ...
            if($chkSession){
                ...
                /**Create Zip Folder ***************/
					...
					// $message .= '<a href = "'.$link.'"><button>'.$new_zip_file.'</button></a>';
                    $subject = 'RING Backup '.date("d-m-Y");
    ...
    public function createReportJsonAndZipForTestingPerpose_21SEP23()
    {
        $data = json_decode(file_get_contents('php://input'));
        $root = (isset($_SERVER['HTTPS']) ? "http://" : "http://") . $_SERVER['HTTP_HOST'];
        $root .= str_replace(basename($_SERVER['SCRIPT_NAME']), '', $_SERVER['SCRIPT_NAME']);
	...
	$AppointmentHistory = isset($data->AppointmentHistory)?$data->AppointmentHistory:"";
            if(!empty($AppointmentHistory)){
                $Appointment = json_encode($AppointmentHistory);
            }else{
                $Appointment = "NULL";
            }
        ...
        if($data)
        {
            ...
            if($chkSession){
                ...
                /**Create Zip Folder ***************/
                    ...
                    $response['response_message'] = 'Success';
                    $response['response_data'] = $returnData;
                    $response['BackupReportID'] = $chkSession[0]->BackupID;
    ...
    function updateBackupDownloadStatus(){
        $data = json_decode(file_get_contents('php://input'));
        if($data)
        {
            $backupId = $data->backupId;
            $patientId = $data->patientId;
            $updateArray = array("IsDownload"=> 1);
            $update = $this->WebserviceModel->updateBackupDownloadStatus($patientId,$backupId,$updateArray);
            if($update == true){
                $response['response_code'] = 1;
                $response['response_message'] ='Success';
            }else{
    ...
    public function saveFileDetailsInBackupFileTableNewLogic(){
        $data = json_decode(file_get_contents('php://input'));       
        $root = (isset($_SERVER['HTTPS']) ? "http://" : "http://") . $_SERVER['HTTP_HOST'];
        $root .= str_replace(basename($_SERVER['SCRIPT_NAME']), '', $_SERVER['SCRIPT_NAME']);
       // print_r($data ); exit ;
    ...
    public function saveBackupReportInTableNewLogic(){
        // echo 123456;exit;
        $data = json_decode(file_get_contents('php://input'));       
        $root = (isset($_SERVER['HTTPS']) ? "http://" : "http://") . $_SERVER['HTTP_HOST'];
        $root .= str_replace(basename($_SERVER['SCRIPT_NAME']), '', $_SERVER['SCRIPT_NAME']);
        if($data)
        {
            ...
            if($ReportUploadType == "local"){
                ...
                $data2 = "";
            }else if($ReportUploadType == "localReport"){
                $data2 = "";
            }
            $description = $data->description;
            $diagnosis = $data->diagnosis;
