<?php
class AppointmentPaymentTest extends CI_Controller
{
   ...
   //public $ringId;
    ...
    {
        parent::__construct();
        $this->load->model(array('WebserviceModel','RingCMSModel'));
    ...
    public function index(){
        ...
        //$AppDetails = $this->RingCMSModel->UserDataByAppointmentId($appId);
        // if(!isset($AppDetails)){
        //     $response['response_code'] = '5';
        //     $response['response_message'] = 'Appointment not available';
        ...
        $payload = [
                          ...
                          "merchant_reference" => "Ring-".$doctorId."-".$ringId."-".time(), 
                          "payment_profile_code" => "", 
                          "instalment_profile_code" => "", 
                          "currency" => "MYR", 
                          "amount" => "700.00", 
                          "description" => "Appointment Payment", 
                          "response_url" => "https://apimobile.ring.healthcare/Ring_Dev/index.php/AppointmentPaymentTest/successResponse", 
                          "payment_update_url" => "", 
                          "callback_url" => "", 
                        //   "additional_reference" => "AP-GHKL001-240923121229-2698", 
                        // "additional_reference" => $ringId."-".$appId."-".$doctorId,
                        "additional_reference" => $ringId."-".$doctorId,
                          "customer" => [
                                "customer_id" =>  $ringId, 
    ...
    function successResponse(){
        ...
        if(isset($_REQUEST["jwt"])){
            ...
            if($transactionData->result_code == 0){
                ...
                $insertArr = array(
                    ...
                    //"AppointmentId"=>$additionalVals[1],
                    // "PractitionerId"=>$additionalVals[2],
                    "PractitionerId"=>$additionalVals[1],
                ...
                $insert = $this->WebserviceModel->insertAppointmentPaymentTransaction($insertArr);
            ...
            // }else if(($transactionData->result_code = 3900) && ($transactionData->payment_status == "CANCELLED")){
            }else if($transactionData->result_code == 3900){
                $additionalData = $jwtDataArray->additional_reference;
                $additionalVals = explode("-",$additionalData);
                $insertArr = array(
                    "PatientId"=>$additionalVals[0],
                ...
                $response['response_data'] = $insertArr;
                //redirect('https://apimobile.ring.healthcare:5026/Ring_Dev/index.php/AppointmentPaymentTest/cancelResponse');
            }else if($transactionData->result_code == 2718){
                //echo "<pre>"; print_r($jwtDataArray);
                $merchantVals = explode("-",$jwtDataArray->merchant_reference);
                $insertArr = array(
                    "PatientId"=>$merchantVals[2],
                    //"AppointmentId"=>$additionalVals[1],
                    "PractitionerId"=>$merchantVals[1],
                    "Status"=>0,
                    "Amount"=>"700.50",
                    "Currency"=>"MYR",
                    "TransactionTimestamp"=>$transactionData->transaction_timestamp,
                ...
                $insert = $this->WebserviceModel->insertAppointmentPaymentTransaction($insertArr);
                $response['response_code'] = '5';
                $response['response_message'] = 'Timeout';
                $response['response_data'] = $insertArr;
                //print_r($response);exit;
                //redirect('https://apimobile.ring.healthcare:5026/Ring_Dev/index.php/AppointmentPaymentTest/timeoutResponse');
            }else{
                $response['response_code'] = '3';
                $response['response_message'] = 'Failled';
                //redirect('https://apimobile.ring.healthcare:5026/Ring_Dev/index.php/AppointmentPaymentTest/failResponse');
            }
