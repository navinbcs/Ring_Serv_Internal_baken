...
//*********START: WEBSERVICE FOR LOGIN***********************
    ...
    function userRegistration(){        
        ...
        if($data)
        {
            ...
            $getRingGroupID = $this->CRModel->getRingGroupID($ringGroup);
            if($getRingGroupID){
    ...
    public function saveReportDataOfCR_User(){
        ...
        if($data)
        {
            ...
            if($ReportUploadType == "local"){
                ...
                $randnum1 = rand(11111111,99999999);  
                $TextFileName = $folderName."/Local_".$randnum1."_".$UniversalID."_".time().".txt";    
            ...
            $filename = "CR_".$randnum."_".$UniversalID."_".time()."_report.json";
    ...
    public function saveFilesDetailsOfCR_User(){
        ...
        {   
			...
			$folderName = "upload/CR_User_Report_Files/";      
            $randnum1 = rand(11111111,99999999);
			$TextFileName = $folderName."/".$randnum1."_".$UniversalID."_".time().".txt";    
            ...
            $randnum = rand(11111111,99999999);
            $filename = "CR_".$randnum."_".$UniversalID."_".time()."_File.json";
            $jsonFileName = $folderName.$filename;        
            $blankArray = array();
            array_push($blankArray, $jsonArr);
            $json = json_encode($blankArray);
    ...
    public function updateReportSyncStatus()
    {
        $data = json_decode(file_get_contents('php://input'));
        if($data)
        {
            ...
            $result = $this->CRModel->updateReportSyncStatus($updateDataArray,$CR_Report_ID); 
            if($result)             
            {
                $response['response_code'] = 1;
                $response['response_message'] = 'Success';
    ...
    public function CloseRingSyncRequest()
    {
        $data = json_decode(file_get_contents('php://input'));
        if($data)
        {
            $UniversalId = $data->UniversalID;
            $UniversalPatientId = $data->PatientId;
            $chkRequest = $this->CRModel->ChkCloseRingSyncStatus($UniversalId);
            if($chkRequest){
                $response['response_code'] = 4;
                $response['response_message'] = 'Sync request already exists.';
            }else{
                $InsertArray = array( 
                    "UniversalId" => $UniversalId,
                    "UniversalPatientId" => $UniversalPatientId,
                    "SyncRequest" => 1
                );
                $result = $this->CRModel->insertData($InsertArray,"CloseRingSync");
                if($result)             
                {
                    $response['response_code'] = 1;
                    $response['response_message'] = 'Success';
                    $response['CR_Sync_Id'] = $result;
                }
                else
                {
                    $response['response_code']=2;
    ...
    public function ChkAvailableCloseRingSyncRequest()
    {
        $data = json_decode(file_get_contents('php://input'));
        if($data)
        {
            $UniversalId = $data->UniversalID;
            $result = $this->CRModel->ChkCloseRingSyncStatus($UniversalId);
            if($result)             
            {
                $response['response_code'] = 1;
                $response['response_message'] = 'Success';
    ...
    public function completeTheSyncRequest()
    {
        $data = json_decode(file_get_contents('php://input'));
        if($data)
        {
            $CR_Sync_Id = $data->CR_Sync_Id;
            $updateDataArray = array( 
                "IsSync" => 1,
                "SyncTime" => date("Y-m-d H:i:s")
            );
            $result = $this->CRModel->completeTheSyncRequest($updateDataArray,$CR_Sync_Id); 
            if($result)             
            {
                $response['response_code'] = 1;
                $response['response_message'] = 'Success';
