...
class CRModel extends CI_Model
{
    ...
    function updateReportSyncStatus($updateDataArray,$CR_Report_ID)
    {
        $this->db->where('ID',$CR_Report_ID);
        $result = $this->db->update('CloseRingReports',$updateDataArray);
        return $result;
    }

    
    public function ChkCloseRingSyncStatus($UniversalId){
        $this->db->select('*, ID as CR_Sync_Id');
        $this->db->from('CloseRingSync');
        $this->db->where('UniversalId',$UniversalId);
        $this->db->where('SyncRequest',1);
        $this->db->where('IsSync',0);
        $result = $this->db->get()->row();
        return $result;
    }

    function completeTheSyncRequest($updateDataArray,$CR_Sync_Id)
    {
        $this->db->where('ID',$CR_Sync_Id);
        $result = $this->db->update('CloseRingSync',$updateDataArray);
        return $result;
    }
