...
/*******************START: APIs***********************/
    ...
    function mergePDFFiles($filenames, $outFile)
    {
        if ($filenames) {
            ...
            $this->mpdf->SetImportUse(); // this line comment out if method doesnt exist
            ...
            foreach ($filenames as $fileName) {
                if (file_exists($fileName)) {
                    $pagesInFile = $this->mpdf->SetSourceFile($fileName);
        ...
        foreach($fileArrayAttachemnt as $fileVal){
            ...
            foreach($fileVal["FileAttachments"] as $fileAttchVal){
                ...
                if($ext == "pdf"){   
                    ...
                    // $str .= '<iframe src="https://apimobile.ring.healthcare/Ring_dev/pdfDoc/'.$pdffilename1.'" width="600px" height="800px" style="border: 0;">
                    //     <p>
                    //         Your browser does not support PDF files.
                    //         <a href="https://apimobile.ring.healthcare/Ring_dev/pdfDoc/'.$pdffilename1.'">Download the file instead</a>
                    //     </p>
                    // </iframe>';
                    // $str .= '</div>';   
                }else{   
    ...
    public function createOnePdfOfAllMRDTRecords(){
        ...
        if($data)
        {
            ...
            if($createPdf)
            {
                ...
                // $response['response_data1'] = $pdfFileArr;
                $response['fileWithPath'] = "https://apimobile.ring.healthcare/Ring_dev/".$createPdf;
            }
            else
            {
                $response['response_code'] = '2';
    ...
    function createPdf($UserId,$dataArray){
        ...
        foreach($dataArray as $key=>$dataValue){
            ...
            $this->mpdf->SetHTMLHeader('<table style="width:100%; margin-top:0; border-collapse: collapse;">
            <tr style="background: #03625c; border-collapse: collapse;">
                <td style="text-align: center; border-collapse: collapse;">
                    <div>
                        <img src="https://apimobile.ring.healthcare:8443/Ring_dev/assets/logo/ring_logo.png" width="100" />
                    </div>
                </td>
                <td style="text-align: right; color:#FFFFFF; border-collapse: collapse;"><div>Created Date'.date("d M Y").'</div></td>
            </tr>
            </table>');
            $this->mpdf->SetHTMLFooter('<table style="width:100%; margin-top:2rem;">
            <tr>
        ...
        if($dataArray->FileArray){
            foreach($dataArray->FileArray as $fileAttchVal){
                ...
                if($ext != "pdf"){   
                    ...
                    // $pdfResult = file_put_contents($path1.$pdffilename1, $curl_data);  
                    // $str .= '<div class="wrapper-table scroll-x margintopcss centerImage">';
                    // $str .= '<embed src="pdfDoc/'.$fileWithPath.'" type="application/pdf" width="100%" height="100%" />';
                    // $str .= '<iframe src="https://apimobile.ring.healthcare/Ring_dev/pdfDoc/'.$pdffilename1.'" width="600px" height="800px" style="border: 0;">
                    //     <p>
                    //         Your browser does not support PDF files.
                    //         <a href="https://apimobile.ring.healthcare/Ring_dev/pdfDoc/'.$pdffilename1.'">Download the file instead</a>
        ...
        if($pdfFileArr){
            foreach($pdfFileArr as $file){
               ...
               // $pdf->addPDF($path1.$file, 'all');
                ...
                $pdf_files[]= $path1.$file;
                // $pdf->addPDF($file, 'all');
            }

              // Merge PDFs and save to output file
           $merged_content = $this->pdfmerger->merge_pdfs($pdf_files); 
           
           $new_file = "pdfDoc/".md5(time().rand(1,10)) .'.pdf';
           $output_file = FCPATH . 'merged_output.pdf'; 
           $this->pdfmerger->save_pdf($merged_content,  $new_file);
          

         // echo "Merged PDF saved as: " . $new_file; exit ;
    ...
    public function test(){
        // require('fpdf.php');APPPATH.'libraries/ion_auth.php'
    ...
    public function createOnePdfOfAllMRDTRecordsWithAllergy1(){
        $data = json_decode(file_get_contents('php://input'));
        $root = (isset($_SERVER['HTTPS']) ? "http://" : "http://") . $_SERVER['HTTP_HOST'];
        $root .= str_replace(basename($_SERVER['SCRIPT_NAME']), '', $_SERVER['SCRIPT_NAME']);
        if($data)
				...
				{  
					...
					foreach($reportArr as $field) {
                        ...
                        if(isset($field->EreferralFormBase64) && !empty($field->EreferralFormBase64)){
							$eform = file_get_contents($field->EreferralFormBase64);
                            $eform1 = $this->getImgUrlFromBase64($eform);
							$field->EreferralFormBase64 = $eform1;
						}
                        $ReportId = $field->BackupReportId;
                        $fileArr = $this->getdataById($reportFileArr, $ReportId);
                        $field->FileArray = $fileArr;
       ...
       // usort($dataArray, 'compareByTimeStamp');
        ...
        foreach($dataArray as $key=>$dataValue){ 
            ...
            if($key == 0){
                ...
                $allergyData =  $allergy;
                $pdfInfo = '<b>Report Date</b> -  '.$sDate.' To '.$eDate.'     <b>Created Date</p> - '.date("d M Y").'    <b> Category</b> - '.$cat.' ';
            }else{
                $userData = "";
                $allergyData = "";
                $pdfInfo = "";
            ...
            if($content){
                ...
                if(isset($dataValue->EreferralFormBase64) && !empty($dataValue->EreferralFormBase64)){
                    $eRefForm = file_get_contents($dataValue->EreferralFormBase64);
                    if(isset($eRefForm) && !empty($eRefForm)){
                    $eRefForm1 = $this->getImgUrlFromBase64($eRefForm);
                    array_push($fileArr,$eRefForm1);
                    }       
                }                      
            }else{
                return null;
    ...
    public function createPdfContentHtmlWithAllergy($content,$dataArray,$userData,$allergy,$pdfInfo,$userAdd){
        // echo "<pre>"; print_r($eRefForm1);exit;
        $filepath = "https://apimobile.ring.healthcare:8443/Ring_dev/upload/";    
        $str ='';
        ...
        if($dataArray->ReportUploadType == "online" || $dataArray->ReportUploadType == "localReport"){
            $str .= '<tr><th>Diagnosis</th><td>'.$dataArray->Diagnosis.'</td></tr>';
            $date = date_create($dataArray->InsertDate);
             $InsertDate = date_format($date,"d M Y h:i A");
            // $timestamp = $dataArray->CreateTime;
    ...
    public function pdfCreationForMedicineAndVaccination($content,$userData,$allergy,$pdfInfo,$userAdd,$medicine,$vaccination){
        ...
        if(isset($allergy) && !empty($allergy)){                     
            $str .= '<tr><th> Allergy</th><td>';
            foreach($allergy as $allergyVal){
                if(isset($allergyVal->Category) && !empty($allergyVal->Category)){
                    $str .= $allergyVal->medicine_name.' - '.$allergyVal->allergy_effect.' ('.$allergyVal->Category.')<br>';
                }else{
                     $str .= $allergyVal->medicine_name.' - '.$allergyVal->allergy_effect.'<br>';
                }               
            }
        ...
        //         $med_startdate = date_create($medicineVal->med_startdate);
        //         $medStartdate = date_format($med_startdate,"d M Y");
        //         $med_enddate = date_create($medicineVal->med_enddate);
        ...
        if(isset($medicine) && !empty($medicine)){                     
            ...
            foreach($medicine as $medicineVal){
                ...
                // $medEnddate = date_format($med_enddate,"d M Y");
                $med_startdate = DateTime::createFromFormat('d/m/Y', $medicineVal->med_startdate);
if ($med_startdate !== false) {
    $medStartdate = $med_startdate->format('d M Y');
} else {
    $medStartdate = 'N/A'; // handle gracefully
}

$med_enddate = DateTime::createFromFormat('d/m/Y', $medicineVal->med_enddate);
if ($med_enddate !== false) {
    $medEnddate = $med_enddate->format('d M Y');
} else {
    $medEnddate = 'Ongoing';
}
                $str .= $medicineVal->medicine.'<br>';
    ...
    public function createOnePdfOfAllMRDTRecordsWithAllergyForMultiLocal(){
        $data = json_decode(file_get_contents('php://input'));
        $root = (isset($_SERVER['HTTPS']) ? "http://" : "http://") . $_SERVER['HTTP_HOST'];
        $root .= str_replace(basename($_SERVER['SCRIPT_NAME']), '', $_SERVER['SCRIPT_NAME']);
        if($data)
